...
const FichaPreview = ({ surgery, onEditSection, autoSave, userProfile }) => {
  ...
  // State to hold fetched CRM of anesthesiologist
  const [anestCrm, setAnestCrm] = useState(null);
  // Fetch anesthesiologist CRM from user collection if needed
  useEffect(() => {
    const fetchAnestCrm = async () => {
      if (!surgery?.createdBy) return;
      try {
        const userDoc = await getUserByUid(surgery.createdBy);
        if (userDoc?.crm) setAnestCrm(userDoc.crm);
      } catch (err) {
        console.error("Erro ao buscar CRM do anestesiologista:", err);
      }
    };
    fetchAnestCrm();
  }, [surgery?.createdBy]);
  ...
  const formatDate = (date) => {
    ...
    // Se a data vier no formato YYYY-MM-DD, evita o problema de fuso horário
          ...
          <div className="border-b border-gray-800 pb-1 mb-2 space-y-1">
            ...
            <div className="flex justify-between text-xs text-gray-600">
              ...
              {console.log("Debug CRM:", {
                ...
                createdByCrm: surgery.createdByCrm,
                userProfileUid: userProfile?.uid,
                userProfileCrm: userProfile?.crm
              })}
              <div>
                <strong>ANESTESIOLOGISTA:</strong> 
                {surgery.createdByName || 'N/A'}
                {anestCrm 
                  ? ` - CRM: ${anestCrm}` 
                  : (userProfile?.uid === surgery.createdBy && userProfile?.crm 
                      ? ` - CRM: ${userProfile.crm}` 
                      : '')}
              </div>
            </div>
