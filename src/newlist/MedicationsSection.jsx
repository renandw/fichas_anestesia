...
const MedicationsSection = ({
  ...
  // Obter lista de medicações do objeto anesthesia
  const medications = anesthesia?.medications || [];
  ...
  const getCaseBaseDate = () => {
    if (anesthesia?.anesthesiaStart instanceof Date) {
      return new Date(anesthesia.anesthesiaStart);
    }
    if (surgery?.metadata?.createdAt instanceof Date) {
      return new Date(surgery.metadata.createdAt);
    }
    return new Date();
  };

  // Helper para aplicar rollover de dia quando o horário escolhido é "menor" que o horário base da cirurgia
  const buildTimestampWithRollover = (base, hh, mm) => {
    const ts = new Date(base);
    const baseHH = base.getHours();
    const baseMM = base.getMinutes();
    ts.setHours(hh, mm, 0, 0);
    // Se o horário escolhido "volta" no relógio em relação ao base, empurra para o dia seguinte
    if (hh < baseHH || (hh === baseHH && mm < baseMM)) {
  ...
  const weightBasedDoses = {
    'Propofol': (weight) => `${Math.round(weight * 3)}mg`,
    'Fentanil': (weight) => `${Math.round(weight * 2)}mcg`,
    'Rocurônio': (weight) => `${Math.round(weight * 0.5)}mg`,
    'Morfina': (weight) => `${Math.round(weight * 0.1)}mcg`,
    'Cetamina': (weight) => `${Math.round(weight * 2)}mg`,
    'Midazolam': (weight) => `${Math.round(weight * 0.15)}mg`,
    'Succinilcolina': (weight) => `${Math.round(weight * 1.0)}mg`,
    'Dipirona': (weight) => `${Math.round(weight * 40)}mg`,         // 15–25 mg/kg IV
    ...
    'Ondansetrona': (weight) => `${Math.round(weight * 0.1)}mg`,     // 0.1 mg/kg, máx 8 mg
    'Dexametasona': (weight) => `${Math.round(weight * 0.1)}mg`,     // 0.1–0.2 mg/kg
    'Dexmedetomidina': (weight) => `${(weight * 1).toFixed(0)}mcg`,  // 1 mcg/kg (dose de ataque)
    'Diazepam': (weight) => `${Math.round(weight * 0.1)}mg`,         // 0.1–0.2 mg/kg IV
    'Haloperidol': (_) => `2.5mg`,                                   // Dose fixa comum IV/IM
    'Cisatracúrio': (weight) => `${Math.round(weight * 0.15)}mg`,    // 0.15–0.2 mg/kg
    'Paracetamol': (weight) => `${Math.round(weight * 15)}mg`,       // 10–15 mg/kg, máx 1g
    'Atracúrio': (weight) => `${Math.round(weight * 0.5)}mg`,        // 0.4–0.5 mg/kg
    'Adrenalina': (weight) => `${(weight * 0.01).toFixed(2)}mg`,     // 0.01 mg/kg em anafilaxia
    'Tramadol': (weight) => `${Math.round(weight * 1.5)}mg`,         // 1–2 mg/kg IV/IM
    ...
    'Ciprofloxacino': (weight) => `${Math.round(weight * 6)}mg`,                               // Dose comum IV
  ...
  // Para PRESETS
  function calculateDoseForPreset(medicationName, originalDose) {
    const weight = surgery?.patientWeight;

    if (!weight || weight > 40 || !weightBasedDoses[medicationName]) return originalDose;
    return weightBasedDoses[medicationName](weight);
  }
  ...
  const medicationPresets = [
    {
      ...
      medications: [
        ...
        { name: 'Cefazolina', dose: '2g', via: 'EV', category: 'Antibiótico' },
        { name: 'Cloreto de Sódio 0,9%', dose: '100mL', via: 'EV', category: 'Cristalóide' },
  ...
  const addPreset = async (preset) => {
    // Calcular horário base do caso + 5 minutos
    const base = getCaseBaseDate();
    const medicationDate = new Date(base.getTime() + 5 * 60 * 1000);
    ...
    const newMedications = preset.medications.map((med, index) => ({
      ...
      time: formatTime(medicationDate),
  ...
  const saveEdit = async (medId) => {
    ...
    // Compute timestamp from editTime and case base date, com rollover de dia
    const base = getCaseBaseDate();
    let tsEdit = new Date(base);
    const chosen = (editTime && /^\d{2}:\d{2}$/.test(editTime)) ? editTime : formatTime(tsEdit);
    const [eh, em] = chosen.split(':').map(Number);
    tsEdit = buildTimestampWithRollover(base, eh, em);

    const updatedMedications = medications.map(med =>
      med.id === medId
        ? { ...med, dose: editDose.trim(), via: editVia.trim(), time: formatTime(tsEdit), timestamp: tsEdit }
        : med
    );
  ...
  const addMedication = async () => {
    ...
    // Novo cálculo de timestamp e hora exibida com base no horário escolhido e data base do caso, com rollover de dia
    const base = getCaseBaseDate();
    let ts = new Date(base);
    if (time && /^\d{2}:\d{2}$/.test(time)) {
      const [hh, mm] = time.split(':').map(Number);
      ts = buildTimestampWithRollover(base, hh, mm);
    }
    const displayTime = time || formatTime(ts);
