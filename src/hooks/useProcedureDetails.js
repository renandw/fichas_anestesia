  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        // 3. Buscar subcoleção surgeries (máximo 1 documento esperado)
        const surgeriesCollectionRef = collection(db, 'patients', patientId, 'procedures', procedureId, 'surgeries');
        unsubscribeSurgery = onSnapshot(
          surgeriesCollectionRef,
          (querySnapshot) => {
            if (!querySnapshot.empty) {
              // Pegar o primeiro documento (deveria haver apenas um por procedure)
              const surgeryDoc = querySnapshot.docs[0];
          ...
          (error) => {
            console.error('Erro ao buscar surgeries:', error);
...
export const useSubCollection = (patientId, procedureId, subCollectionName) => {
  ...
  useEffect(() => {
    ...
    // Mapear nome da subcoleção para o nome correto no Firestore
    let collectionName = subCollectionName;
    if (subCollectionName === 'surgery') {
      collectionName = 'surgeries';
    }

    const collectionRef = collection(db, 'patients', patientId, 'procedures', procedureId, collectionName);
...
export const useSubCollectionOperations = (patientId, procedureId) => {
  ...
  const updateSubCollectionDocument = async (subCollectionName, documentId, data) => {
    try {
      ...
      const docRef = doc(db, 'patients', patientId, 'procedures', procedureId, collectionName, documentId);
